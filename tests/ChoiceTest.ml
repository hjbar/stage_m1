...
let process = function
  | String str -> (
    ...
    with ChoicePathParser.Error ->
      Printf.ksprintf failwith "ChoiceTest: parse error on input %S" str )
...
let () =
  Arg.parse spec (fun s -> raise (Arg.Bad s)) "a test utility for choice paths";
