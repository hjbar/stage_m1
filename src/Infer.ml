...
module Make(T : Utils.Functor) = struct
  ...
  *)
    let rec has_type (env : env) (t : Untyped.term) (w : variable) : (STLC.term, err) t =
        let fresh = Constraint.Var.fresh in
        let fresh_from x = fresh (Untyped.Var.name x) in
        match t with
        | Untyped.Var x -> (
        ...
        | Untyped.Abs (x, t) -> (
            let x_var = fresh_from x
            and t_var = fresh "warr"
            and res_var = fresh "wt"
            in Exist (x_var, None,
                Exist (res_var, None,
        ...
        | Untyped.Tuple ts -> (
            ...
            and t1_var = fresh "w1"
            and t2_var = fresh "w2"
            and prod_var = fresh "prod"
            in Exist (t1_var, None,
                Exist (t2_var, None,
        ...
        | Untyped.LetTuple (xs, t, u) -> (
            ...
            let x1_var = fresh_from x1
            and x2_var = fresh_from x2
            and prod_var = fresh "wt"
            in Exist (x1_var, None,
                Exist (x2_var, None,
